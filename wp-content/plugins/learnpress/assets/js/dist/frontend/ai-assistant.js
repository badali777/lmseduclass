...
"use strict";
/*!************************************************!*\
  !*** ./assets/src/js/frontend/ai-assistant.js ***!
  \************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   AIAssistantWidget: () => (/* binding */ AIAssistantWidget)
/* harmony export */ });
...
class AIAssistantWidget {
  ...
  validateDOM() {
    ...
    return !!(this.elements.toggleBtn && this.elements.panel && this.elements.msgList);
  }
  applyInitialState() {
    if (this.elements.smartReviewBtn) {
      const showSmartReview = this.config.context === 'quiz' ? this.config.quizCompleted : !!this.config.enabledActions?.smart_review;
  ...
  handleSendClick(args) {
    var _this$elements$inputE;
    args.e.preventDefault();
    this.sendMessage((_this$elements$inputE = this.elements.inputEl?.value) !== null && _this$elements$inputE !== void 0 ? _this$elements$inputE : '');
  }
  ...
  handleInputKeydown(args) {
    ...
    if (args.e.key === 'Enter' && !args.e.shiftKey) {
      var _this$elements$inputE2;
      args.e.preventDefault();
      this.sendMessage((_this$elements$inputE2 = this.elements.inputEl?.value) !== null && _this$elements$inputE2 !== void 0 ? _this$elements$inputE2 : '');
    }
  }
  ...
  loadHistory() {
    try {
      ...
      const lastReview = [...this.history].reverse().find(item => item?.type === 'quiz_review' && item?.review);
      this.lastQuizReviewSignature = lastReview?.review ? this.getQuizReviewKey(lastReview.review) : '';
    } catch (_e) {
      this.history = [];
      this.lastQuizReviewSignature = '';
    }
  ...
  appendMessage(role, text) {
    ...
    el.innerHTML = `<span class="lp-ai-assistant__msg-label">${this.escHtml(label)}</span>` + `<p class="lp-ai-assistant__msg-text">${this.escHtml(text)}</p>`;
  ...
  getQuizReviewFromState(quiz) {
    var _quiz$feedback$select, _quiz$feedback$correc;
    ...
    const selectedIndex = Number.parseInt((_quiz$feedback$select = quiz.feedback.selected_index) !== null && _quiz$feedback$select !== void 0 ? _quiz$feedback$select : -1, 10);
    const correctIndex = Number.parseInt((_quiz$feedback$correc = quiz.feedback.correct_index) !== null && _quiz$feedback$correc !== void 0 ? _quiz$feedback$correc : -1, 10);
    return {
  ...
  getQuizReviewKey(review) {
    return [review.question_index, review.total, review.selected_index, review.correct_index, review.is_correct ? 1 : 0].join('|');
  }
  ...
  appendQuizReviewCard(review) {
    ...
    const feedbackTitle = review.is_correct ? this.config.i18n.quizCorrectTitle : this.config.i18n.quizWrongTitle;
    const feedbackHtml = `<div class="lp-ai-assistant__quiz-feedback ${feedbackClass}">` + `<strong>${this.escHtml(feedbackTitle)}</strong>` + (review.explanation ? `<div>${this.escHtml(review.explanation)}</div>` : '') + '</div>';
    card.innerHTML = `<div class="lp-ai-assistant__quiz-head">Question ${review.question_index + 1}/${review.total || review.options.length}</div>` + `<div class="lp-ai-assistant__quiz-question">${this.escHtml(review.question || '')}</div>` + `<div class="lp-ai-assistant__quiz-options">${this.renderQuizReviewOptions(review)}</div>` + feedbackHtml;
  ...
  renderQuizState() {
    ...
    card.innerHTML = `<div class="lp-ai-assistant__quiz-head">Question ${currentIndex + 1}/${quiz.total || options.length}</div>` + `<div class="lp-ai-assistant__quiz-question">${this.escHtml(question.question || '')}</div>` + `<div class="lp-ai-assistant__quiz-options">${optionsHtml}</div>`;
    this.elements.msgList.appendChild(card);
...
_utils_js__WEBPACK_IMPORTED_MODULE_0__.lpOnElementReady(AIAssistantWidget.selectors.root, () => {
